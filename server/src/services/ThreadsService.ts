...
import {
  // CreateThreadSchema,
  UpdateThreadSchema
} from "../validator/ThreadsValidator";
  ...
  async findAll(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (limit) {
        ...
        const findThreadsByUser = await this.threadRepository.find({
        ...
        findThreadsByUser.forEach((Thread) => {
          Thread.image = Thread.image && `${Thread.image}`
        })
        if (!findThreadsByUser) {
          return res.status(404).json({ error: "No threads available from this user" })
        }
        return res.status(200).json(findThreadsByUser)
      } else {
